...
```{r}
acc <- function(truth, predictions){
  100*sum(truth==predictions)/length(truth)
}

acc(train_df$Survived, naiveModel(train_df))
...
```{r}
library(rpart)
m0 = rpart(Survived ~ Age + Pclass + Sex + SibSp + Fare, train_df)

predictions = data.frame(truth=train_df$Survived, response=round(predict(m0, train_df)))
acc(predictions$truth, predictions$response)
```

### Przygotowujemy wyniki do wysłania
```{r}

submission_df %>% dplyr::mutate(Survived = round(predict(m0, submission_df))) %>%
